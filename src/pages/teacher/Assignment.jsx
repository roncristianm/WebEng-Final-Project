// src/pages/teacher/Assignment.jsx  (UPDATED — edit support added)
...
import { updateAssignment } from '../../services/editService'
    ...
    <div ref={ref} style={{ position: 'relative', width: '100%' }}>
      <button
        ...
        style={{
          ...
          border: `2px solid ${open ? '#0038A8' : '#e2e8f0'}`, borderRadius: 12,
          background: '#fff', cursor: 'pointer', display: 'flex',
          justifyContent: 'space-between', alignItems: 'center', fontFamily: 'inherit',
          color: selected ? '#1e293b' : '#9ca3af', fontWeight: selected ? 600 : 400,
          boxSizing: 'border-box', boxShadow: open ? '0 0 0 3px rgba(0,56,168,0.1)' : 'none',
      ...
      {open && (
        <div style={{
          ...
          boxShadow: '0 8px 28px rgba(0,0,0,0.13)', zIndex: 1000, maxHeight: 280,
          overflowY: 'auto', overflowX: 'hidden',
        }}>
          {classes.length === 0 && (
            <div style={{ padding: '16px', color: '#9ca3af', fontSize: 14, textAlign: 'center' }}>No classes found</div>
          )}
          {classes.map((cls, i) => {
...
function Assignment() {
  ...
  // Edit state
  const [editTarget, setEditTarget] = useState(null)   // assignment object being edited
  ...
  const handleSaveEdit = async (fields) => {
    ...
    if (result.success) {
      ...
      setEditTarget(null)
      // If detail modal is open for this assignment, refresh it
      if (showDetailModal && selectedAssignment?.id === editTarget.id) {
        const fresh = await getAssignmentById(editTarget.id)
          ...
          <div className="modal-content modal-large" onClick={e=>e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div style={{ display:'flex', justifyContent:'flex-end', marginBottom:12 }}>
                <button
                  onClick={() => { setShowDetailModal(false); setEditTarget(selectedAssignment) }}
                  style={{
