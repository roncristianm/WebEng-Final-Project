...
app.get('/debug-tabs', async (req, res) => {
  try {
    ...
    console.error('debug-tabs error:', safeError(err))
...
app.get('/tab-names', async (req, res) => {
  try {
    ...
    console.error('tab-names error:', safeError(err))
    res.status(500).json({ error: 'Internal server error' })
  }
})
...
app.get('/student-grades', async (req, res) => {
  try {
    ...
    console.error('student-grades error:', safeError(err))
    res.status(500).json({ error: 'Internal server error' })
  }
})
...
app.post('/set-highest-possible-score', async (req, res) => {
  try {
    ...
    console.error('set-highest-possible-score error:', safeError(err))
    res.status(500).json({ error: 'Internal server error' })
  }
})
...
app.post('/repair-hps', async (req, res) => {
  try {
    ...
    console.error('repair-hps error:', safeError(err))
    res.status(500).json({ error: 'Internal server error' })
  }
})
...
app.post('/record-score', async (req, res) => {
  try {
    ...
    if (itemNumber !== undefined && itemNumber !== null) {
      ...
      else return res.status(400).json({ error: 'Unknown assignmentType' })
    ...
    console.error('record-score error:', safeError(err))
    res.status(500).json({ error: 'Internal server error' })
  }
})

// ── Delete assignment column from sheet ───────────────────────────────────────
app.post('/delete-assignment-from-sheet', async (req, res) => {
  try {
    const { sheetId, assignmentType, assignmentId, itemNumber, quarter = 'Q1' } = req.body
    ...
    if (itemNumber !== undefined && itemNumber !== null) {
      ...
      if (colIndex === -1)
        return res.status(404).json({ error: 'Could not locate assignment column to clear.' })
    }
...
app.post('/update-assignment-in-sheet', async (req, res) => {
  try {
    ...
    async function getHeaders(quarter) {
      const safeQ     = VALID_QUARTERS.has(quarter) ? quarter : 'Q1'
      const sheetName = getQuarterTab(tabs, safeQ)
      if (!sheetName) return null
      const r    = await sheets.spreadsheets.values.get({ spreadsheetId: sheetId, range: `${sheetName}!A8:AZ9` })
    ...
    batchClear.push({ range: `${oldHeaders.sheetName}!${oldColLetter}${SHEET_HPS_ROW}`, values: [['']] })
    batchWrite.push({ range: `${newHeaders.sheetName}!${newColLetter}${SHEET_HPS_ROW}`, values: [[newA.possibleScore]] })

    if (somethingMoved && studentScores.length > 0) {
      const oldAllRows = await getAllRows(oldHeaders.sheetName)
      const newAllRows = quarterChanged ? await getAllRows(newHeaders.sheetName) : oldAllRows
      ...
      for (const { studentName, score } of studentScores) {
        ...
        batchClear.push({ range: `${oldHeaders.sheetName}!${oldColLetter}${oldRowIdx + 1}`, values: [['']] })
    ...
    console.error('update-assignment-in-sheet error:', safeError(err))
    res.status(500).json({ error: 'Internal server error' })
  }
})

// ── Clear a student's score cells ─────────────────────────────────────────────
app.post('/clear-student-scores', async (req, res) => {
  try {
    ...
    for (const a of assignments) {
      ...
      byQuarter.get(q).push(a)
    }

    const clearData = []

    for (const [quarter, qAssignments] of byQuarter.entries()) {
    ...
    console.error('clear-student-scores error:', safeError(err))
    res.status(500).json({ error: 'Internal server error' })
  }
})
...
// ── Remove student row from all quarter grade tabs ────────────────────────────
app.post('/remove-student-from-quarter-sheets', async (req, res) => {
  try {
    ...
    for (const quarter of safeQuarters) {
      const sheetName = getQuarterTab(tabs, quarter)
      if (!sheetName) continue
      ...
      if (studentRowIdx === -1) { results.push({ quarter, status: 'not found' }); continue }

      const cleanName = sheetName.replace(/^'|'$/g, '')
      const tabProp   = sheetProps.find(p => p.title === cleanName)
      if (!tabProp) { results.push({ quarter, status: 'tab not found in sheet metadata' }); continue }
      ...
      await sheets.spreadsheets.batchUpdate({
        ...
        requestBody: {
          requests: [{
            deleteDimension: {
              range: {
                sheetId:    tabProp.sheetId,
                dimension:  'ROWS',
                startIndex: studentRowIdx,
                endIndex:   studentRowIdx + 1
              }
            }
          }]
        }
    ...
    console.error('remove-student-from-quarter-sheets error:', safeError(err))
    res.status(500).json({ error: 'Internal server error' })
  }
})
...
app.post('/append-grade', async (req, res) => {
  try {
    ...
    console.error('append-grade error:', safeError(err))
...
app.put('/update-cell', async (req, res) => {
  try {
    ...
    console.error('update-cell error:', safeError(err))
    res.status(500).json({ error: 'Internal server error' })
  }
})
